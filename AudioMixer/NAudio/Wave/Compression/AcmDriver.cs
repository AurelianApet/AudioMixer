...
namespace NAudio.Wave.Compression
{
    ...
    public class AcmDriver : IDisposable
    {
        ...
        {
            IntPtr handle = Utils.NativeMethods.LoadLibrary(driverFile);
            if (handle == IntPtr.Zero)
            {
                throw new ArgumentException("Failed to load driver file");
            }
            var driverProc = Utils.NativeMethods.GetProcAddress(handle, "DriverProc");
            if (driverProc == IntPtr.Zero)
            {
                Utils.NativeMethods.FreeLibrary(handle);
        ...
        public static void RemoveLocalDriver(AcmDriver localDriver)
        {
            ...
            Utils.NativeMethods.FreeLibrary(localDriver.localDllHandle);
