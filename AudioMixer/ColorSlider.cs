...
namespace Video.Controls
{
    ...
    public partial class ColorSlider : Control
    {
        ...
        private Color barOuterColor = Color.DimGray;
        /// <summary>
        ...
        private Color barInnerColor = Color.DimGray;
        /// <summary>
        ...
        private Color barPenColor = Color.DimGray;
        /// <summary>
        ...
        private Color elapsedOuterColor = Color.DimGray;
        /// <summary>
        ...
        private Color elapsedInnerColor = Color.DimGray;
        /// <summary>
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                if (mouseEffects && mouseInRegion && false)
                {
                                     ...
                                     Color barPenColorPaint, Color elapsedOuterColorPaint, Color elapsedInnerColorPaint)
        {
            try
            {
                ...
                if (barOrientation == Orientation.Horizontal)
                {
                    ...
                    thumbRect = new Rectangle(TrackX, 1, thumbSize - 1, ClientRectangle.Height - 2);
                }
                else
                {
                    int TrackY = (((trackerValue - barMinimum) * (ClientRectangle.Height - thumbSize)) / (barMaximum - barMinimum));
                    thumbRect = new Rectangle(1, TrackY, ClientRectangle.Width - 2, thumbSize - 1);
                }
                ...
                if (barOrientation == Orientation.Horizontal)
                {
                    if (!IsPulse) barRect.Inflate(0, 0);
                    else barRect.Inflate(-1, -barRect.Height / 3);
                    barHalfRect = barRect;
                ...
                {
                    barRect.Inflate(0, 0);// (-barRect.Width / 3, -1);
                    barHalfRect = barRect;
                    barHalfRect.Width /= 2;
                ...
                using (
                    LinearGradientBrush lgbBar =
                        new LinearGradientBrush(barHalfRect, IsPulse? barInnerColorPaint:barOuterColorPaint, barInnerColorPaint, gradientOrientation)
                    )
                {
                ...
                if (Focused & drawFocusRectangle && false)
        ...
        public void UpdateMouseWheelOnOther(MouseEventArgs e)
        {
            int v = e.Delta / 120 * Math.Max(50, (barMaximum - barMinimum) / 100);
            SetProperValue(Value - v);
        }
        ...
        /// <returns></returns>
        public static GraphicsPath CreateRoundRectPath(Rectangle rect, Size size, int t = 0)
        {
            GraphicsPath gp = new GraphicsPath();
            gp.AddLine(rect.Left + size.Width / 2, rect.Top, rect.Right - size.Width / 2, rect.Top);
            if (t > 1)
            {
                gp.AddLine(rect.Right - size.Width / 2, rect.Top, rect.Right, rect.Top);
                gp.AddLine(rect.Right, rect.Top, rect.Right, rect.Top + size.Height / 2);
            }
            else gp.AddArc(rect.Right - size.Width, rect.Top, size.Width, size.Height, 270, 90);

            gp.AddLine(rect.Right, rect.Top + size.Height / 2, rect.Right, rect.Bottom - size.Width / 2);
            if (t > 1)
            {
                gp.AddLine(rect.Right, rect.Bottom - size.Width / 2, rect.Right, rect.Bottom);
                gp.AddLine(rect.Right, rect.Bottom, rect.Right - size.Width / 2, rect.Bottom);
            }
            else gp.AddArc(rect.Right - size.Width, rect.Bottom - size.Height, size.Width, size.Height, 0, 90);

            gp.AddLine(rect.Right - size.Width / 2, rect.Bottom, rect.Left + size.Width / 2, rect.Bottom);
            if (t == 1 || t==3)
            {
                gp.AddLine(rect.Left + size.Width / 2, rect.Bottom, rect.Left, rect.Bottom);
                gp.AddLine(rect.Left, rect.Bottom, rect.Left, rect.Bottom + size.Width / 2);
            }
            else gp.AddArc(rect.Left, rect.Bottom - size.Height, size.Width, size.Height, 90, 90);

            gp.AddLine(rect.Left, rect.Bottom - size.Height / 2, rect.Left, rect.Top + size.Height / 2);
            if (t == 1 || t == 3)
            {
                gp.AddLine(rect.Left, rect.Top + size.Height / 2, rect.Left, rect.Top);
                gp.AddLine(rect.Left, rect.Top, rect.Left + size.Width / 2, rect.Top);
            }
            else gp.AddArc(rect.Left, rect.Top, size.Width, size.Height, 180, 90);
            return gp;
        }
